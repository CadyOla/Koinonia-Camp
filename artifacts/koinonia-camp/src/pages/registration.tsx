...
export default function Registration() {
  ...
  // Registration can be turned off (see src/routes/registration-status.ts /
  // REGISTRATIONS_OPEN env var). null = still checking, so the form/closed
  // screen doesn't flash before we know which to show.
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/registration-status`)
      ...
      .catch(() => {
        // Fail open on a transient network error — the backend's own
        // POST /registrations guard still enforces the real block if
        // registrations are actually closed, so this only risks briefly
        // showing the form, never actually accepting a submission when closed.
  ...
  // Still checking whether registration is open — avoid flashing the form
  // (or the closed screen) before we actually know.
    ...
    return (
      <div className="min-h-[100dvh] w-full bg-background flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-[2rem] shadow-xl p-8 text-center">
          ...
          <p className="text-muted-foreground mb-8 text-lg">
            {closedMessage ||
              "We're no longer accepting new registrations for Koinonia Camp 2026."}
          </p>
          <a
            href="/my-registration"
            className="inline-flex items-center justify-center w-full h-12 rounded-xl bg-primary text-primary-foreground font-medium text-lg hover:bg-primary/90 transition-colors"
          >
