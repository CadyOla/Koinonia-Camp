...
export default function AdminDashboard() {
  ...
  const handleExport = () => {
    ...
    const exportData = registrations.map((r) => ({
      ...
      "Lodging Type": r.lodgingType || "N/A",
      "Roommate Preferences": r.roommatePreferences || "N/A",
      "Special Needs": r.specialNeeds || "N/A",
      Feeding: r.feedingPreference,
      Transport: r.transportPreference,
      "Payment Status": r.paymentStatus || "N/A",
      "Room Assignment": r.roomAssignment || "N/A",
      "Bus Assignment": r.busAssignment || "N/A",
      "HQ Booking ID": r.hqBookingId || "N/A",
      "HQ Synced At": r.hqSyncedAt
        ? format(new Date(r.hqSyncedAt), "MMM d, yyyy h:mm a")
        : "N/A",
      "Confirmation SMS Sent At": r.smsSentAt
        ? format(new Date(r.smsSentAt), "MMM d, yyyy h:mm a")
        : "N/A",
      "Room Invite SMS Sent At": r.roomSmsSentAt
        ? format(new Date(r.roomSmsSentAt), "MMM d, yyyy h:mm a")
        : "N/A",
      "Payment Reminder SMS Sent At": r.paymentSmsSentAt
        ? format(new Date(r.paymentSmsSentAt), "MMM d, yyyy h:mm a")
        : "N/A",
      "Meal - Friday Evening": r.mealFridayEvening || "N/A",
      "Meal - Saturday Afternoon": r.mealSaturdayAfternoon || "N/A",
      "Meal - Saturday Evening": r.mealSaturdayEvening || "N/A",
      "Meal - Sunday Afternoon": r.mealSundayAfternoon || "N/A",
      "Meal - Sunday Evening": r.mealSundayEvening || "N/A",
      "Meal - Monday Brunch": r.mealMondayBrunch || "N/A",
      "Meal Choices Submitted At": r.mealSelectionsSubmittedAt
        ? format(new Date(r.mealSelectionsSubmittedAt), "MMM d, yyyy h:mm a")
        : "N/A",
      "Date Registered": format(new Date(r.createdAt), "MMM d, yyyy h:mm a"),
      "Last Updated": format(new Date(r.updatedAt), "MMM d, yyyy h:mm a"),
    }));
    ...
    const wscols = [
      ...
      { wch: 15 }, // Room Type
      { wch: 15 }, // Lodging Type
      { wch: 25 }, // Roommate Preferences
      { wch: 25 }, // Special Needs
      { wch: 15 }, // Feeding
      { wch: 15 }, // Transport
      { wch: 15 }, // Payment Status
      { wch: 20 }, // Room Assignment
      { wch: 15 }, // Bus Assignment
      { wch: 14 }, // HQ Booking ID
      { wch: 20 }, // HQ Synced At
      { wch: 20 }, // Confirmation SMS Sent At
      { wch: 20 }, // Room Invite SMS Sent At
      { wch: 24 }, // Payment Reminder SMS Sent At
      { wch: 30 }, // Meal - Friday Evening
      { wch: 32 }, // Meal - Saturday Afternoon
      { wch: 30 }, // Meal - Saturday Evening
      { wch: 22 }, // Meal - Sunday Afternoon
      { wch: 26 }, // Meal - Sunday Evening
      { wch: 22 }, // Meal - Monday Brunch
      { wch: 22 }, // Meal Choices Submitted At
  ...
  const handleHqSync = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(err.error || "HQ sync failed to start. Check the backend logs.");
      }
    } catch {
      alert("Something went wrong. Please try again.");
    } finally {
      ...
      <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="flex flex-col items-end gap-2">
            ...
            {hqSyncResult && (
              <div className="text-xs text-muted-foreground">
                <p>
                  HQ sync done: {hqSyncResult.matched} matched, {hqSyncResult.updated} updated ({hqSyncResult.reassigned} switched Resident/Non-Resident, {hqSyncResult.locked} locked/skipped), out of {hqSyncResult.fetched} fetched.
                </p>
                {hqSyncResult.unmatched.length > 0 && (
                  <div className="mt-1">
                    <p className="font-medium">
                      {hqSyncResult.unmatched.length} unmatched (in HQ's data, no matching local registrant — check phone/email):
                    </p>
                    <ul className="list-disc list-inside">
                      {hqSyncResult.unmatched.map((u) => (
                        <li key={u}>{u}</li>
                      ))}
                    </ul>
                  </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <StatCard
            ...
            value={stats?.successfullyRegistered}
            subtitle={
              stats
                ? `${stats.successfullyRegisteredResident} Resident · ${stats.successfullyRegisteredNonResident} Non-Resident`
                : undefined
            }
            icon={UserCheck}
        ...
        {stats && stats.needsRoomAssignment.length > 0 && (
          <Card className="border-none shadow-sm border-l-4 border-l-amber-400">
            <CardContent className="p-4">
              <p className="text-sm font-semibold text-amber-800 mb-1">
                {stats.needsRoomAssignment.length} Resident
                {stats.needsRoomAssignment.length === 1 ? "" : "s"} paid but
                haven't picked a room yet
              </p>
              <p className="text-xs text-muted-foreground mb-2">
                Payment is confirmed, but there's no synced room — they may
                not have finished HQ's accommodation step yet.
              </p>
              <div className="flex flex-wrap gap-2">
                {stats.needsRoomAssignment.map((r) => (
                  <span
            ...
            <Card className="border-none shadow-sm">
              <CardHeader className="border-b px-6 py-4 bg-gray-50/30">
                ...
                {!isLoadingStats && stats && (
                  <p className="text-xs text-muted-foreground mt-1">
                    {stats.mealSelectionsSubmitted} of{" "}
                    {stats.mealSelectionsEligible} Church Feeding registrants
                    submitted
                  </p>
                )}
              </CardHeader>
              <CardContent className="p-0">
                <div className="max-h-[420px] overflow-y-auto">
                  ...
                  ) : !stats?.mealBreakdown?.some((s) => s.counts.length > 0) ? (
                    <div className="p-6 text-center text-sm text-muted-foreground">
                      No submissions yet
                    </div>
                  ) : (
                    <div className="divide-y divide-gray-100">
                      {stats.mealBreakdown.map((slot) => (
                        <div key={slot.slot} className="p-4">
                          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">
                            {slot.slot}
                          </p>
                          {slot.counts.length === 0 ? (
                            ...
                            <div className="space-y-2">
                              {slot.counts.map((c) => (
                                ...
                                >
                                  <span className="text-sm text-foreground line-clamp-1 flex-1 pr-4">
                                    {c.label}
                                  </span>
                                  <span className="text-sm font-bold text-primary bg-primary/5 px-2 py-1 rounded-md">
                                    {c.count}
