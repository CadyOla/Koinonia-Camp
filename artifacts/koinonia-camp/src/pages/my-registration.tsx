                  ...
                  <>
                    ...
                    <div>
                      {MEAL_SLOTS.map((slot, i) => {
                        const [day, timeOfDay] = slot.label.split(" ");
                        const prevDay =
                          i > 0 ? MEAL_SLOTS[i - 1].label.split(" ")[0] : null;
                        const isNewDay = day !== prevDay;
                        const isLast = i === MEAL_SLOTS.length - 1;
                        const TimeIcon = timeOfDay === "Evening" ? Moon : Sun;
                        const collectedAt = (data as any)[
                          `${slot.key}CollectedAt`
                        ] as string | null | undefined;
                        return (
                          <div key={slot.key} className="flex gap-3">
                            <div className="flex flex-col items-center w-14 flex-shrink-0">
                              <span className="text-xs font-semibold text-secondary h-4">
                                {isNewDay ? day : ""}
                              </span>
                              <div className="flex flex-col items-center mt-1 flex-1">
